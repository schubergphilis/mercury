...
func ipPing(proto string, h models.Healthcheck) (models.Status, error) {
	errorcount := 0
	errormsg := ""
	for i := 0; i < h.PINGpackets; i++ {
		_, _, err := pingAddr(proto, h.TargetIP, h.TargetPort, h.SourceIP, i, 64, time.Duration(h.PINGtimeout)*time.Second)
		if err != nil {
			errorcount++
			errormsg = err.Error()
		}
	...
	if errorcount == h.PINGpackets {
		return models.Offline, fmt.Errorf("%s ping lost 100%% packets: %s", proto, errormsg)
	}
