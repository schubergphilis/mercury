...
func (manager *Manager) HealthHandler(healthCheck *healthcheck.Manager) {
	...
	for {
		...
		case checkresult := <-healthCheck.Incoming:
			...
			log.WithField("pool", checkresult.PoolName).WithField("backend", checkresult.BackendName).WithField("node", checkresult.NodeName).WithField("online", checkresult.Online).WithField("check", checkresult.Description).Info("Received health update from worker")

			// Set status in healh pool
			healthCheck.SetCheckStatus(checkresult.WorkerUUID, checkresult.Online, checkresult.ErrorMsg)
			...
			for _, nodeUUID := range nodeUUIDs {
				...
				checkresult.Online = actualStatus
